    ...
    <div className="h-full overflow-auto bg-base-100 p-4 sm:p-6 lg:p-8">
        ...
        <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
          <div>
            ...
            <p className="mt-2 max-w-2xl text-sm sm:text-base opacity-90">
              Track your orders, save your favorite restaurants, and discover
              fresh meals that match your cravings.
            </p>
          </div>
          <Link
            to="/order-now"
            className="inline-flex items-center justify-center gap-2 rounded-sm bg-base-100 px-4 py-2 text-sm font-semibold text-primary transition hover:opacity-90"
          >
      ...
      <div className="mt-6 grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        {stats.map((item, index) => (
          <div
            key={index}
            className="rounded-lg border border-base-200 bg-base-100 p-4 shadow-sm flex justify-between"
          >
            <div>
            ...
            <div
              className={`mb-3 flex h-10 w-10 items-center justify-center rounded-full ${item.color}`}
            >
              {item.icon}
            </div>
          </div>
        ...
        <div className="rounded-lg border border-base-200 bg-base-100 p-6 shadow-sm">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-sm text-secondary">
                Popular spots picked for your taste.
              </p>
            </div>
            <Link
              to="/order-now"
          ...
          <div className="mt-4 space-y-3">
            {restaurants.slice(0, 3).map((restaurant) => (
              <div
                key={restaurant.id}
                className="flex items-center justify-between rounded-2xl border border-base-200 bg-base-100 p-4"
              >
                <div>
                  ...
                  <div className="mt-1 flex flex-wrap items-center gap-2 text-sm text-secondary">
                    <span className="inline-flex items-center gap-1">
                      <FiStar className="text-amber-500" /> {restaurant.rating}
                    </span>
                    <span className="inline-flex items-center gap-1">
                      <FiMapPin /> {restaurant.city}
                    </span>
                  </div>
                </div>
                <Link
        ...
        <div className="space-y-6">
          <div className="rounded-lg border border-base-200 bg-base-100 p-6 shadow-sm">
            ...
            <ul className="mt-4 space-y-3 text-sm text-secondary">
              <li className="rounded-lg bg-base-100 p-3">
                Your last order from{" "}
                <span className="font-semibold text-base-content">
                  Under The Mango Tree
                </span>{" "}
                was delivered 20 mins ago.
              </li>
              <li className="rounded-lg bg-base-100 p-3">
                You saved{" "}
                <span className="font-semibold text-base-content">
                ...
                to your wishlist this week.
              </li>
              <li className="rounded-lg  bg-base-100 p-3">
                New offer available for{" "}
                <span className="font-semibold text-base-content">
                  free delivery
                </span>{" "}
                on your next order.
